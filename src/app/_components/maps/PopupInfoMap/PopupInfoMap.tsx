...
const PopupInfoMap = () => {
  ...
  const [marker, setMarker] = React.useState<PopupInfoProps>();
  
  const handleMarkerClick = (targetMarker: PopupInfoProps) => {
    setMarker({ ...targetMarker, showInfo: true });
  };

  const handleMarkerClose = (targetMarker: PopupInfoProps) => {
          ...
          {data?.map((item, index) => (
            ...
            >
              {marker?.showInfo && item.slug === marker.slug && (
                <InfoWindow onCloseClick={() => handleMarkerClose(item)}>
